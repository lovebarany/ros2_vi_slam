#ifndef VI_MONO_NODE_HPP
#define VI_MONO_NODE_HPP
...
class VIMonoNode : public rclcpp::Node
{
    public:
            VIMonoNode(); // Constructor
            ~VIMonoNode(); // Destructor
            void virtual track(); // Check for new images, and track with these and IMU measurements saved

    private:
            std::mutex mBufMutexIm, mBufMutexImu; // Mutexes for access to image and IMU queues/vectors
            std::queue<sensor_msgs::msg::Image::SharedPtr> imgBuf; // Queues to contain incoming camera images
            std::vector<ORB_SLAM3::IMU::Point> imuMeasurements; // Vector of IMU measurements since last camera frame

            void image_callback(const sensor_msgs::msg::Image::SharedPtr msg); // Callback to save image sent 
